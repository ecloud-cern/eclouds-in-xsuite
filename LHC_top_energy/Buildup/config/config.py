...
for Dh_sc in [0.2e-3, 0.3e-4, 0.5e-4, 0.1e-4]:
    for key in eclouds_info.keys():
        ...
        #current_sim_ident = f"{tag_prefix}_{ecloud_type.upper()}_{ecloud_index}_sey{del_max:.2f}_{fact_beam/1e11:.2f}e11ppb_3x48"
        ...
        rl.replaceline_and_save(fname = config_dir + "simulation_parameters.input",
                                findln = "Dt =", newline = f"Dt = {Dt:e}\n")
        rl.replaceline_and_save(fname = config_dir + "simulation_parameters.input",
                                findln = "Dh_sc =", newline = f"Dh_sc = {Dh_sc:e}\n")
