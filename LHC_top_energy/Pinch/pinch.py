...
parser.add_argument("--ecloud", nargs="?", type=str, default="ecloud.q1r5.ir5.0")
parser.add_argument("--sey", nargs="?", type=float, default=1.35)
...
pinch_name = f"LHC6.8TeV_v1_{ecloud_type.upper()}_{index}_sey{sey:.2f}_1.20e11ppb"

out_pinch = f"{pinch_name}.h5"
optics_conf = "run3_bet30cm_160urad_1.2e11ppb_2.0um"
eos_folder = "/eos/project/e/ecloud-simulations/kparasch/LHC_Triplets/"
                

buildup_folder = eos_folder + f"/Buildup/{optics_conf}/simulations_SEY{sey:.2f}/{pinch_name}/"
pinch_folder = eos_folder + f"/Pinches/{optics_conf}/SEY{sey:.2f}/"

this_folder = f"{pinch_name}/"
os.makedirs(f"simulations/{this_folder}", exist_ok=True)
os.chdir(f"simulations/{this_folder}")
...
# shutil.copyfile(out_pinch, pinch_folder + out_pinch )
# os.chdir(f"..")
